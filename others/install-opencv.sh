...
cmake -DWITH_CUDA=ON -DWITH_QT=ON -DFORCE_VTK=ON -DWITH_TBB=ON -DWITH_GDAL=ON \
      -DWITH_XINE=ON -DENABLE_PRECOMPILED_HEADERS=OFF ..
fi

if [ $OPENCV_CONTRIB = 'YES' ]; then
cmake -DWITH_CUDA=ON -DWITH_QT=ON -DFORCE_VTK=ON -DWITH_TBB=ON \
      -DWITH_GDAL=ON -DWITH_GTK_2_X=OFF\
      -DWITH_XINE=ON -DENABLE_PRECOMPILED_HEADERS=OFF \
